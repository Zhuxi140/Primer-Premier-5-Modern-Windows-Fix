...
两个数字都是对的，只是描述的对象不同。补丁涉及两个连续区域，**跨度合计 37 字节**，
但其中**实际发生值变化的只有 33 字节**，因为 code cave 内有 4 个字节在原文件中
本来就是所需的值。

```text
区域 A —— 偏移 0x154F5 - 0x154FE （跨度 10 字节，变化 10 字节）
  原始 : 8B 95 7C FF FF FF 83 7A 54 00
  补丁 : E9 E6 39 03 00 90 90 90 90 90

区域 B —— 偏移 0x48EE0 - 0x48EFA （跨度 27 字节，变化 23 字节）
  原始 : 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
  补丁 : 8B 95 7C FF FF FF 81 FA 00 00 01 00 0F 82 AA C6 FC FF 83 7A 54 00 E9 04 C6 FC FF
```
...
它们在原文件中是 `00`，补丁写入的也是 `00`。写入内容与原值完全相同，
因此在这些偏移处观察不到任何变化。
