...
**第一步 —— 判断这个补丁适不适用于你。** 先算一下你自己那份 DLL 的 SHA256：

```powershell
Get-FileHash "C:\Primer Premier 5\xnmba458.dll" -Algorithm SHA256
```

如果结果正好是下面这个值，说明补丁支持你的版本：
...
**第二步 —— 下载并运行补丁脚本。** 取
[`patch/patch-xnmba458.ps1`](patch/patch-xnmba458.ps1)（也可以直接从
[最新 Release](../../releases/latest) 下载附件），然后执行：

```powershell
.\patch-xnmba458.ps1 -Path "C:\Primer Premier 5\xnmba458.dll"
```

脚本会自动备份你的 DLL、逐字节校验、应用补丁，并再次校验结果。
任何一项检查不通过，它都会**在写入任何字节之前中止**；如果是在写入之后才失败，
它会**自动把原始 DLL 还原回去**。不存在「尽力而为」模式。

不确定哈希是否匹配，或者遇到的是别的报错？请看
[两个互相独立的问题](#两个互相独立的问题) 和
...
**实际发生变化的字节共 33 个**，位于两个**区域跨度为 37 字节**（10 + 27）的区间内。
两个数字都是对的，原因见下方说明。

```text
区域 A —— 文件偏移 0x154F5，跨度 10 字节
  原始：8B 95 7C FF FF FF 83 7A 54 00
  修改：E9 E6 39 03 00 90 90 90 90 90
  实际变化：10 / 10 字节

区域 B —— 文件偏移 0x48EE0，跨度 27 字节（原文件为零填充）
...
**为什么是 33 而不是 37？** 区域 B 中有 4 个字节 —— `0x48EE8`、`0x48EE9`、
`0x48EEB`、`0x48EF5` —— 在原文件里本来就是所需的值（`00`）。
它们被写入了相同的字节，因此值没有变化。
